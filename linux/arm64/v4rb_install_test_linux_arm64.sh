...
DEB_FILE="v4rb_arm64_${MAJOR_VERSION}_lin.deb"

# Download the specified VERSION of the V4RB for Linux ARM64
...
# Copy the V4RB shared library to the ARM64 test project
cp "$V4RB_INSTALL_DIR/ValentinaPlugin/ValentinaPlugin.xojo_plugin/Valentina/Build Resources/Linux arm64/v4rb_arm64_release.so" 'linux/arm64/TestProjectConsole/TestProjectConsole Libs'

# Run the V4RB application and capture the output
OUTPUT=$(linux/arm64/TestProjectConsole/TestProjectConsole)
